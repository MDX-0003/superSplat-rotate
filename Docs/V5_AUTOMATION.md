# SuperSplat 自动化渲染管线 (v5) — 技术文档

> 最后更新: 2026-06-26

## 1. 目标

将 SuperSplat 网页端的 PLY 加载 + 相机导入 + 视频渲染三个步骤完全自动化，
...
| `subprocess` | 调用现有的 fus_ply / clip_ply / ffmpeg |
| `pathlib` | 跨平台路径处理 |
| `json` | pipeline.json 读写 |

## 4. 完整流程

### 4.1 Config 文件

每个项目一个 `CameraData/<project>/pipeline.json`，由原来的 `presets.json` + `tl_*.json` 合并而成：
...
```json
{
  ...
  "interpolate": { "total": 250, "anchor_camera": "001", "radius_scale": 1.0 },
  "fuse": { "indices": [1,2], "radius_scale": 1, "height_up": 3, ... },
  "clip": { "clip_percent": 0.0, "denoise": true, ... },
  "output": {
...
关键设计：
- `segments` 中 `render` 类型不再需要 `seq` 字段——从前一个 `real` segment 的 `end+1` 自动推导文件名（如 `seq_f075.mp4`）
- `jsons_path` 可选，指向存放相机 JSON 的文件夹；不存在时跳过 JSON 导入步骤
- 所有旧文件（`presets.json`, `tl_*.json`, `ply_pipeline.py`）保持不变，向后兼容

### 4.2 步骤总览

```
Step 1  [auto] interpolate → cameras_align.json       (调用 tills_ply/)
Step 2  [auto] fuse → combine PLYs                     (调用 tills_ply/)
Step 3  [auto] clip → XX-clip/*.ply                    (调用 tills_ply/)
Step 4a [交互] 列出 XX-clip/*.ply → 用户输入 idx
Step 4b [交互] 列出 cameras_folder/*.json → 用户输入 idx  (可选)
Step 5a [auto] Playwright 启动 Chrome + 上传 PLY
Step 5b [auto] 上传 JSON → 自动导入 GT+Timeline         (可选)
Step 6  [auto] 验证时间轴就绪
Step 7  [auto] 视频渲染 (OPFS 流式) + 分段读回下载
Step 8  [auto] 提取实拍帧 → 编码 MP4 → TS concat → output.mp4
```

### 4.3 Step 5a — PLY 上传细节
...
async def render_video(page, total_frames, renders_dir, expected_filename, fps):
    ...
    # 3. 轮询进度 (window.__renderProgress)

    # 4. 渲染完成后，从 OPFS 分段读回 (10 MB/chunk)
...
### 5.2 `page.wait_for_event("download")` 不触发
...
**正确做法**：OPFS 流式写入 + 分段读回（见 §2.4, §4.5），完全绕过下载机制。

### 5.3 `render.video` 使用 BufferTarget → 4K 视频 OOM
...
`functions` 是内部的 `Map<string, Function>`，可以从外部 `delete` 再注册——但 TypeScript class field
在 Rollup 编译后属性名可能被混淆，不保证可访问。
...
| 问题 | 根因 | 修复 |
|------|------|------|
| `step()` 缺 `shell` 参数 | v4 的 `step()` 没有 `shell` 参数但 v5 的副本有 | 补上 `shell=False` |
| `timeline.frames` 默认 180 | SuperSplat 新建场景就有 180 帧空时间轴 | 改用 `camera.importedPoses().length` 判断是否已导入相机 |
| Concat `DTS out of order` | 实拍 `yuvj420p(pc)` vs 渲染 `yuv420p(tv)` 色彩范围不同，TS 时间戳断裂 | 编码实拍帧时加 `-vf "scale=iw:ih:out_range=tv"` + concat demuxer |

## 6. 命令行参考

```bash
# 完整流程（需要先 npm run serve 启动 dev server）
python tills/run_pipeline_v5.py --config CameraData/01/pipeline.json

# 仅 PLY 处理（interpolate → fuse → clip）
python tills/run_pipeline_v5.py --config CameraData/01/pipeline.json --steps ply

# 仅渲染 + concat（跳过 PLY 处理）
...
| `src/file-handler.ts` | SuperSplat 源码，修改了 `cameraImportSessionMode` 默认值 |
| `tills_ply/*.py` | PLY 处理子脚本（被 v5 调用，保持独立可用） |
