...
Usage:
  # CLI only
  python tills/fuse_ply.py --path CameraData/04 --max-index 89 --height-up 0.6 --height-down 0.5 --radius-scale 0.5 --bias

  # with config file (auto-discovered or explicit)
  python tills/fuse_ply.py
  python tills/fuse_ply.py --config my_config.json
...
# ---------------------------------------------------------------------------
# circle fitting (same algorithm as interpolate_cameras_circle.py)
# ---------------------------------------------------------------------------
...
def write_ply(filepath: str, header_lines, properties, vertex_blocks):
    """Write a binary little-endian PLY merging one or more (N_i, P) vertex arrays."""
    if vertex_blocks:
        all_verts = np.vstack(vertex_blocks)
    else:
