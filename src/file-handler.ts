...
const transformPlyRotation = (rotation: number[][]) => {
    ...
    // 3DGS/COLMAP cameras look along +Z with +Y down; PlayCanvas cameras look
    // along -Z with +Y up.
    xAxis.set(-rotation[0][0], -rotation[1][0], rotation[2][0]);
    yAxis.set(rotation[0][1], rotation[1][1], -rotation[2][1]);
    zAxis.set(rotation[0][2], rotation[1][2], -rotation[2][2]);

    rotationMat.data.set([
    ...
    return rotationQuat.setFromMat4(rotationMat).clone();
};
    ...
    if (json.length > 0) {
        ...
        json.sort(sorter).forEach((pose: any, i: number) => {
            if (pose.hasOwnProperty('position') && pose.hasOwnProperty('rotation')) {
                ...
                const position = transformPlyPosition(p);
                const rotation = transformPlyRotation(pose.rotation);
