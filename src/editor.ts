...
const registerEditorEvents = (events: Events, editHistory: EditHistory, scene: Scene) => {
    ...
    events.on('camera.align', (axis: string) => {
        scene.camera.releaseCalibratedPoseForInteraction();
    ...
    events.function('camera.getPose', () => {
        ...
        const focal = Math.max(targetSize.width, targetSize.height) /
        ...
        return {
            ...
            fov: camera.fov,
            rotation: camera.poseRotation?.clone() ?? camera.mainCamera.getLocalRotation().clone(),
            intrinsics: camera.poseIntrinsics ? { ...camera.poseIntrinsics } : {
    ...
    events.on('camera.setPose', (pose: { name?: string, position: Vec3, target: Vec3, fov?: number, rotation?: any, intrinsics?: { width: number, height: number, fx: number, fy: number } }, speed = 1) => {
        ...
        if (pose.rotation && pose.intrinsics) {
            scene.camera.setCalibratedPose(pose.position, pose.target, pose.rotation, pose.intrinsics, speed);
            events.fire('gtReference.clear');
        } else {
            scene.camera.clearCalibratedPose();
            scene.camera.setPose(pose.position, pose.target, speed);
            events.fire('gtReference.clear');
        }
        scene.camera.onUpdate(0);
        (scene.getElementsByType(ElementType.splat) as Splat[]).forEach((splat) => {
            if (splat.visible) {
                splat.entity.gsplat.instance.sort(scene.camera.mainCamera);
            }
        });
        scene.forceRender = true;
    });
