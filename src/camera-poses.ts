...
const deriveRotationFromOrbitPose = (pose: Pose) => {
    const direction = new Vec3();
    direction.sub2(pose.target, pose.position);
    const length = direction.length();
    if (length < 1e-8) {
        return new Quat();
    }

    const azim = Math.atan2(-direction.x / length, -direction.z / length) * 180 / Math.PI;
    const elev = Math.asin(direction.y / length) * 180 / Math.PI;
    return new Quat().setFromEulerAngles(elev, azim, 0);
};
...
class CameraAnimTrack implements AnimTrack {
    ...
    constructor(events: Events) {
        ...
        events.on('timeline.frame', (frame: number) => {
            // During playback `timeline.frame` is only the integer UI frame.
            // The camera itself should follow the fractional `timeline.time`.
            if (!this.events.invoke('timeline.playing')) {
                this.evaluate(frame);
            }
        });
    ...
    addKey(frame: number): boolean {
        ...
        const newPose: Pose = {
            ...
            intrinsics: pose.intrinsics ? { ...pose.intrinsics } : undefined
        };
    ...
    copyKey(fromFrame: number, toFrame: number): boolean {
        ...
        this.poses.push({
            ...
            intrinsics: source.intrinsics ? { ...source.intrinsics } : undefined
        });
    ...
    addPose(pose: Pose): void {
        ...
        pose.intrinsics = pose.intrinsics ? { ...pose.intrinsics } : undefined;
    ...
    private rebuildSpline(): void {
        ...
        const fallbackTargetSize = this.events.functions.has('targetSize') ?
            this.events.invoke('targetSize') :
            { width: 1920, height: 1080 };
        const fallbackFov = this.events.functions.has('camera.fov') ?
            this.events.invoke('camera.fov') :
            60;

        const getPoseRotation = (pose: Pose) => {
            return pose.rotation ?? deriveRotationFromOrbitPose(pose);
        };
        ...
        if (orderedPoses.length > 1) {
            ...
            const findSegment = (frame: number) => {
                ...
                if (loopSpan > 0 && (frame >= last.frame || frame <= first.frame)) {
                    return {
                        ...
                        b: first,
                        t: frame >= last.frame ?
                            (frame - last.frame) / loopSpan :
                            (duration - last.frame + frame) / loopSpan
                    };
                }
            ...
            this.onTimelineChange = (frame: number) => {
                ...
                pose.fov = result[6];
                pose.rotation = undefined;
                pose.intrinsics = undefined;

                const segment = findSegment(frame);
                if (segment) {
                    const { a, b, t } = segment;
                    const ar = getPoseRotation(a);
                    const br = getPoseRotation(b);
                    const ai = getPoseIntrinsics(a);
                    const bi = getPoseIntrinsics(b);
                    const width = lerp(ai.width, bi.width, t);
                    const height = lerp(ai.height, bi.height, t);

                    interpolatedRotation.slerp(ar, br, t);
                    pose.rotation = interpolatedRotation.clone();
                    pose.intrinsics = {
                        width,
                        height,
                        fx: lerp(ai.fx / ai.width, bi.fx / bi.width, t) * width,
                        fy: lerp(ai.fy / ai.height, bi.fy / bi.height, t) * height
                    };
                }
            ...
            this.onTimelineChange = () => {
                this.events.fire('camera.setPose', orderedPoses[0], 0);
            };
        } else {
            this.onTimelineChange = null;
        }
...
const registerCameraPosesEvents = (events: Events) => {
    ...
    events.function('camera.importedPoses', () => {
        return importedPoses;
    });
        ...
        return [{
            ...
            poses: poses.map((pose) => {
                return {
                    ...
                    fov: pose.fov,
                    rotation: pose.rotation ? [pose.rotation.x, pose.rotation.y, pose.rotation.z, pose.rotation.w] : undefined,
                    intrinsics: pose.intrinsics ? { ...pose.intrinsics } : undefined
                };
            })
        }];
    ...
    events.function('docDeserialize.poseSets', (poseSets: any[], documentCameraFov?: number) => {
        ...
        const loadedPoses: Pose[] = poseSets[0].poses.map((docPose: any, index: number) => {
            return {
                ...
                rotation: docPose.rotation ? new Quat(docPose.rotation) : undefined,
                intrinsics: docPose.intrinsics ? { ...docPose.intrinsics } : undefined
            };
        });
