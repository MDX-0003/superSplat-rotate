...
class EditorUI {
    ...
    constructor(events: Events) {
        ...
        const gtReference = document.createElement('img');
        gtReference.id = 'gt-reference';
        gtReference.hidden = true;
        gtReference.alt = '';
        gtReference.draggable = false;
        ...
        canvasContainer.dom.appendChild(gtReference);
        ...
        events.on('gtReference.set', (name?: string) => {
            if (!name) {
                gtReference.hidden = true;
                gtReference.removeAttribute('src');
                return;
            }

            const imageName = `${name}.jpg`;
            gtReference.src = `/gt-images/${encodeURIComponent(imageName)}`;
            gtReference.hidden = false;
        });

        events.on('gtReference.clear', () => {
            gtReference.hidden = true;
            gtReference.removeAttribute('src');
        });
