...
class Camera extends Element {
    ...
    offscreenSize: { width: number, height: number } = null;
    poseIntrinsics: { width: number, height: number, fx: number, fy: number } | null = null;
    posePosition: Vec3 | null = null;
    poseRotation: Quat | null = null;
    ...
    setPose(position: Vec3, target: Vec3, dampingFactorFactor: number = 1) {
        this.clearCalibratedPose();
        this.setOrbitPose(position, target, dampingFactorFactor);
    }
    ...
    setCalibratedPose(position: Vec3, target: Vec3, rotation: Quat, intrinsics: { width: number, height: number, fx: number, fy: number }, dampingFactorFactor: number = 1) {
        ...
        this.posePosition = position.clone();
        this.applyTargetSizeForCurrentMode();
        this.setOrbitPose(position, target, dampingFactorFactor);
        this.posePosition = position.clone();
        this.poseRotation = rotation.clone();
        this.mainCamera.setLocalRotation(this.poseRotation);
        this.fitClippingPlanes(this.mainCamera.getLocalPosition(), this.mainCamera.forward);
    }

    clearCalibratedPose() {
        this.posePosition = null;
        this.poseRotation = null;
        this.poseIntrinsics = null;
        this.camera.calculateProjection = null;
        this.applyTargetSizeForCurrentMode();
    }

    releaseCalibratedPoseForInteraction() {
        if (this.posePosition || this.poseRotation || this.poseIntrinsics) {
            this.clearCalibratedPose();
        }
    }

    setOrbitPose(position: Vec3, target: Vec3, dampingFactorFactor: number = 1) {
        this.poseRotation = null;
    ...
    onUpdate(deltaTime: number) {
        ...
        if (this.posePosition) {
            cameraPosition.copy(this.posePosition);
        } else if (this.lookCameraPos) {
        ...
        if (this.poseIntrinsics) {
            const source = this.poseIntrinsics;
            const width = targetSize.width;
            const height = targetSize.height;
            const fx = source.fx * width / source.width;
            const fy = source.fy * height / source.height;
    ...
    focus(options?: { focalPoint: Vec3, radius: number, speed: number }) {
        this.releaseCalibratedPoseForInteraction();
    ...
    setTargetSizeOverride(width: number, height: number) {
        this.targetSizeOverride = { width, height };
        this.rebuildRenderTargets();
        this.onUpdate(0);
    }

    clearTargetSizeOverride() {
        this.targetSizeOverride = null;
        this.rebuildRenderTargets();
        this.onUpdate(0);
    }

    applyTargetSizeForCurrentMode() {
        const size = this.offscreenSize ?? this.poseIntrinsics;
        if (size) {
            this.setTargetSizeOverride(size.width, size.height);
        } else if (this.targetSizeOverride) {
