...
const registerRenderEvents = (scene: Scene, events: Events) => {
    ...
    events.function('render.batchGtCameras', async (poses: readonly Pose[], width: number, height: number) => {
        events.fire('startSpinner');

        try {
            events.fire('progressStart', localize('panel.view-options.gt-camera.exporting'), true);
            ...
            try {
                ...
                for (let i = 0; i < total; i++) {
                    ...
                    downloadFile(arrayBuffer, `${pose.name}.png`);

                    events.fire('progressUpdate', {
                        text: localize('panel.view-options.gt-camera.exporting-progress', { current: i + 1, total }),
                        progress: total > 1 ? 100 * (i + 1) / total : 100
                    });
                }
    ...
    events.function('render.video', (videoSettings: VideoSettings, fileStream: FileSystemWritableFileStream) => {
        const renderImpl = async () => {
            ...
            try {
                ...
                const frameCount = Math.floor(duration * frameRate + 1e-6) + 1;
                ...
                for (let frameIndex = 0; frameIndex < frameCount; frameIndex++) {
                    ...
                    const timelineFrame = frameIndex === frameCount - 1 ?
                        endFrame :
                    ...
                    events.fire('progressUpdate', {
                        ...
                        progress: frameCount > 1 ? 100 * frameIndex / (frameCount - 1) : 100
                    });
                }
